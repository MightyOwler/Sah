...
def registracija_post():
    ...
    if uporabnik:
        ...
        napaka = model.PrikazovanjeStrani.registracija_doloci_napako(
            uporabnisko_ime)

    if napaka:
        return bottle.template("registracija.tpl", napaka=napaka)
    else:
        vse_skupaj = model.VseSkupaj.vnesi_novega_uporabnika(
            uporabnisko_ime, geslo_v_cistopisu, vse_skupaj)
        bottle.response.set_cookie(
            "uporabnisko_ime", uporabnisko_ime, path="/", secret=SKRIVNOST)
        bottle.redirect("/")
...
def igraj_proti_racunalniku_post():
    ...
    napaka = model.PrikazovanjeStrani.igraj_proti_cloveku_doloci_napako(
        beli=beli, crni=crni)
    if napaka:
...
def shrani_igro():
    ...
    vse_skupaj = model.PrikazovanjeStrani.shrani_igro(
        vse_skupaj, igra=igra, uporabnisko_ime=uporabnisko_ime, beli=beli, crni=crni, celoten_fen=celoten_fen)
    bottle.redirect("/")
...
def arhiv_get():
    ...
    slovar_rezultatov, slovar_rezultatov_s_podatki = model.PrikazovanjeStrani.statistika(
...
def server_static(id):
    ...
    uporabnik = poisci_uporabnika(uporabnisko_ime)
    vse_uporabnikove_igre = uporabnik.igre
    igra, beli, crni, popravljen_celoten_fen = model.PrikazovanjeStrani.arhiv_igra(
        id, vse_uporabnikove_igre=vse_uporabnikove_igre)
